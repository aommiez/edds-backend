(function( Popcorn ) {
  ...
  // HTML5 media's play and pause are asynch, but do fire in sequence
  ...
  // it will default to using an HTML5 media in the target.
  Popcorn.smart = function( target, src, options ) {
    var node = typeof target === "string" ? Popcorn.dom.find( target ) : target,
        ...
        // of the mix, since we'll default to HTML5 media if nothing
    ...
    // Default to using HTML5 media.  Similar to the HTMLVideoElement
    // wrapper, we put a media in the div passed to us via:
    // Popcorn.smart( div, src, options )
    var videoHTML,
        videoElement,
        videoID = Popcorn.guid( "popcorn-media-" ),
    ...
    // IE9 doesn't like dynamic creation of source elements on <media>
    // so we do it in one shot via innerHTML.
    videoHTML = '<media id="' +  videoID + '" preload=auto autobuffer>';
    for ( i = 0, srcLength = src.length; i < srcLength; i++ ) {
