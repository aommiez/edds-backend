(function( Popcorn, window, document ) {
  ...
  function HTMLYouTubeVideoElement( id ) {
    ...
    function onPlayerReady( event ) {
      ...
      var onMuted = function() {
        if ( player.isMuted() ) {
          // force an initial play on the media, to remove autostart on initial seekTo.
    ...
    function onPlayerError(event) {
      ...
      switch( event.data ) {
        ...
        // invalid parameter
        case 2:
          err.message = "Invalid media parameter.";
        ...
        // media can't be embedded by request of owner
        case 101:
        case 150:
    ...
    function onReady() {
      ...
      // Ensure media will now be unmuted when playing due to the mute on initial load.
      if( !impl.muted ) {
        player.unMute();
      }
    ...
    function changeSrc( aSrc ) {
      ...
      // Remove the media id, since we don't want to pass it
      delete playerVars.v;
      ...
      // Don't show media info before playing
      ...
      // Set wmode to transparent to show media overlays
      ...
      // Get media ID out of youtube url
    ...
    function onPlay() {
      ...
      if( playerPaused ) {
        ...
        // Only 1 play when media.loop=true
    ...
    function onEnded() {
      if( impl.loop ) {
        ...
        impl.ended = true;
        onPause();
        // YouTube will fire a Playing State change after the media has ended, causing it to loop.
  ...
  // We'll attempt to support a mime type of media/x-youtube
  Popcorn.HTMLYouTubeVideoElement.canPlayType = function( type ) {
    return type === "media/x-youtube" ? "probably" : EMPTY_STRING;
  };
