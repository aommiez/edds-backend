...
var testData = {
  ...
  playerSpecificSyncTests: function() {
    ...
    test( "YouTube 03 - _canPlaySrc", 6, function() {
      ...
      ok( !Popcorn.HTMLYouTubeVideoElement._canPlaySrc( "http://www.youtube.com" ), "Youtube can't play http://www.youtube.com without a media id" );
      ok( !Popcorn.HTMLYouTubeVideoElement._canPlaySrc( "www.youtube.com" ), "Youtube can't play www.youtube.com without a media id" );
      start();
    });
...
// div. Simulate the same effect by deleting all iframes under #media
// after each test ends.
var qunitStart = start;
start = function() {
  // Give the media time to finish loading so callbacks don't throw
  setTimeout( function() {
    qunitStart();
