...
// TODO: more tests from http://w3c-test.org/html/tests/approved/media
...
asyncTest( "T22 - Setting src loads media, triggers loadedmetadata, sets currentSrc", 1, function() {
...
asyncTest( "T23 - media.duration, durationchagne event", 1, function() {
...
asyncTest( "T26 - loop (NOTE: test takes a minute to complete) [Known to fail with Vimeo+Firefox (ticket #1267)]", 3, function() {
  ...
  function onTimeUpdate() {
    // Fast-forward toward the end of the media when at the start
...
/** This fails in all browsers, so I think spec has maybe changed...
  media.src = testData.videoSrc;
  notEqual( media.currentSrc, "", "currentSrc is not empty after setting src" );
**/
...
test( "T30 - error when media parameter is bad", function() {
  ...
  video.src = "data:media/x-fake;base64,0";
...
asyncTest( "T34 - paused state during autoplay", 10, function() {
  ...
  video.addEventListener( "loadedmetadata", function onLoadedMetaData() {
    ...
    ok( !video.paused, "media is playing during loadedmetadata" );
  }, false);
  ...
  video.autoplay = true;
  ok( video.paused, "media does not autoplay before source" );
  video.src = testData.videoSrc;
  ok( video.paused, "media does not autoplay before ready events" );
...
asyncTest( "T35 - duration ready in loadedmetadata", 1, function() {
  ...
  video.addEventListener( "loadedmetadata", function onLoadedMetaData() {
    video.removeEventListener( "loadedmetadata", onLoadedMetaData, false );
    ok( video.duration, "media is playing during loadedmetadata" );
    start();
  }, false);
  video.src = testData.videoSrc;
});
