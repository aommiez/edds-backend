...
asyncTest( "Subtitle", function() {
  ...
  popped.cue( 3, function() {
    ...
    this.on( "pause", function onPause() {
      this.off( "pause", onPause );
      // check position of subtitle that should have moved with media,
      // a subtitle must be displayed to get valid data
      ...
      // check values against the media's values
      // we need four checks because if we just check against media's position,
      // and media's position hasn't updated either, we'll pass when we should fail
...
asyncTest( "Media element as target", 2, function() {
  ...
  containerAfterParse = document.getElementById( "media-overlay" );

  ok( !!containerAfterParse, "media-overlay container exists" );
  equal( containerAfterParse.children[ 0 ].innerHTML, "Text on media", "Text displayed in created overlay container" );
